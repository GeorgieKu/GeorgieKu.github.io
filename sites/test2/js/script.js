(function () {
    ...
    const renderDrawer = () => {
        ...
        // «Перейти к оформлению» — ссылка на отдельную страницу,
        // поэтому блокируем её классом, а не атрибутом disabled
        const checkout = document.querySelector('.cart__checkout');
        if (checkout) {
            checkout.classList.toggle('is-disabled', isEmpty);
            checkout.setAttribute('aria-disabled', isEmpty ? 'true' : 'false');
        }
    };
    ...
    // «Вернуться в корзину» на странице оформления открывает шторку корзины
