function openAcc(toggleButton, content, contents) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    toggleButtons.forEach((toggleButton, index) => {
        openAcc(toggleButton, contents[index], contents);
    });
});

document.addEventListener('DOMContentLoaded', function() {
    let buttons = document.querySelectorAll('.acc__item button');
...
selects.forEach(function (select) {
    select.addEventListener('click', function () {
        ...
        let selected = select.querySelector('.select__content span');
        let selectItems = select.querySelectorAll('.select__list li');
...
if (calendarContainer) {
    const datepicker = new AirDatepicker('#calendar-input', {
    ...
    const openBtn = document.getElementById('calendar-open-btn');
...
const dateSwiper5 = document.querySelector('.date-swiper-5');
...
if (dateSwiper5) {
    const wrapper = dateSwiper5.querySelector('.swiper-wrapper');
    wrapper.innerHTML = '';
    ...
    dates.forEach((item, index) => {
        ...
        slide.innerHTML = `
        <div class="date-swiper__block">
            <span class="date-swiper__date">${item.date}</span>
            <span class="date-swiper__day">${item.dayOfWeek}</span>
            <span class="date-swiper__text">
                4
                <svg width="14" height="6" viewBox="0 0 14 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M10.5283 1.06348C10.6635 0.793913 11.0783 0.813278 11.1738 1.12109L11.5186 2.23535H12.6465C12.9812 2.23542 13.1244 2.66104 12.8584 2.86426L11.9307 3.57227L12.2832 4.70898C12.3838 5.0345 12.0071 5.29747 11.7363 5.09082L10.8398 4.40527L9.94336 5.09082C9.67258 5.29747 9.29589 5.0345 9.39648 4.70898L9.74805 3.57227L8.82129 2.86426C8.55527 2.66104 8.69848 2.23542 9.0332 2.23535H10.1611L10.5059 1.12109L10.5283 1.06348Z" fill="#FF4D00" stroke="#C2E912" stroke-width="0.4"/>
                    <path d="M8.1875 1.24219C8.35343 0.910967 8.82626 0.910967 8.99219 1.24219L10.6699 4.59766C10.8194 4.89678 10.6019 5.24888 10.2676 5.24902H6.91211C6.57774 5.24888 6.36028 4.89678 6.50977 4.59766L8.1875 1.24219Z" fill="#0DAD0D" stroke="#C2E912" stroke-width="0.4"/>
                    <rect x="3.73359" y="0.966016" width="4.00156" height="4.00156" rx="1.2" fill="#6B41E9" stroke="#C2E912" stroke-width="0.4"/>
                    <rect x="0.70625" y="0.966016" width="4.00156" height="4.00156" rx="2.00078" fill="#008CFF" stroke="#C2E912" stroke-width="0.4"/>
                </svg>
            </span>
        </div>
      `;
    ...
    var Swipes = new Swiper('.date-swiper-5', {
        loop: false,
        spaceBetween: 9,
        slidesPerView: 'auto',
        navigation: {
            nextEl: '.date-next-btn-5',
            prevEl: '.date-prev-btn-5',
            clickable: true,
        },
    });
}
let achiveModal = document.getElementById('achiveModal');


function openAchiveModal() {
    modals.forEach(function (modal) {
        modal.style.display = 'none';
    })
    achiveModal.style.display = 'block';
    changeBurger();
...
function openCityModal() {
    ...
    let mathcesSwiper = document.querySelector('.closest-matches__swiper');

    if (mathcesSwiper) {
        var Swipes = new Swiper('.closest-matches__swiper', {
            loop: false,
            spaceBetween: 8,
            slidesPerView: 'auto',
        });
...
let createMatchModal = document.getElementById('createMatchModal');
...
let courtSwiper = document.querySelector('.court-swiper');

if (courtSwiper) {
    var Swipes = new Swiper('.court-swiper', {
        loop: false,
        spaceBetween: 20,
        slidesPerView: 'auto',
    });
...
const dateSwiper1 = document.querySelector('.date-swiper-1');
    ...
    if (dateSwiper1) {
        const wrapper = dateSwiper1.querySelector('.swiper-wrapper');
        wrapper.innerHTML = '';
      ...
      `;
        ...
        var Swipes = new Swiper('.date-swiper-1', {
            loop: false,
            spaceBetween: 9,
            slidesPerView: 'auto',
            navigation: {
                nextEl: '.date-next-btn-1',
                prevEl: '.date-prev-btn-1',
                clickable: true,
            },
        });
    }
...
document.querySelectorAll('.btns-block').forEach(block => {
    block.querySelectorAll('button').forEach(button => {
        button.addEventListener('click', () => {
            block.querySelectorAll('button.active').forEach(btn => btn.classList.remove('active'));
...
function deleteAvatar() {
    let avatar = document.querySelector('.profile-about .avatar img');
...
const input = document.querySelector('#donateModal input[type="text"]');
const strong = document.querySelector('#donateModal strong');
...
input.addEventListener('input', () => {
  ...
  if (input.value.length > 0) {
    ...
    const activeBtn = document.querySelector('.donate-modal__flex button.active');
    if (activeBtn) {
      strong.textContent = activeBtn.textContent;
    } else {
      strong.textContent = '';
    }
  }
...
let footerBall = document.getElementById('footerBall');
let footerMenu = document.getElementById('footerNav');


if (footerBall) {
    footerBall.addEventListener('click', function () {
...
function openInviteMatchModal() {
        modals.forEach(function (modal) {
        modal.style.display = 'none';
    })
    inviteMatchModal.style.display = 'block';
    document.querySelector('body').style.overflow = 'hidden'
    changeBurger()
      scrollToTop();
      footerMenu.classList.remove('active');
...
const dateSwiper4 = document.querySelector('.date-swiper-4');
    ...
    if (dateSwiper4) {
        const wrapper = dateSwiper4.querySelector('.swiper-wrapper');
        wrapper.innerHTML = '';
      ...
      `;
        ...
        var Swipes = new Swiper('.date-swiper-4', {
            loop: false,
            spaceBetween: 9,
            slidesPerView: 'auto',
            navigation: {
                nextEl: '.date-next-btn-4',
                prevEl: '.date-prev-btn-4',
                clickable: true,
            },
        });
    }
...
let matchesModal = document.getElementById('matchesModal');
...
let notificationsBtn = document.querySelector('.notifications-modal__btn');

notificationsBtn.addEventListener('click', function() {
    let notificationsList = document.querySelector('.notifications-modal__list');
    let notificationsText = document.querySelector('.notifications-modal__no-content');
    notificationsList.style.opacity = '0';
    notificationsText.style.display = 'block'

    notificationsBtn.setAttribute('disabled', 'disabled')
...
document.querySelectorAll('.top-tabs button').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        document.querySelectorAll('.tab-content').forEach(tc =>
            tc.classList.toggle('active', tc.classList.contains(btn.dataset.tab))
        );
    });
});

document.querySelectorAll('.tab-content').forEach(tabBlock => {
    let innerBtns = tabBlock.querySelectorAll('.inner-tabs button');
    ...
    innerBtns.forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            btn.classList.add('active');
            let contents = tabBlock.querySelectorAll('.inner-content');
            contents.forEach(c =>
                c.classList.toggle('active', c.classList.contains(btn.dataset.innerTab))
            );
        });
    });
});
...
const playoffBtns = {
    sixteenthFinal: document.querySelector('.playoff button[value="1/16"]'),
    eighthFinal: document.querySelector('.playoff button[value="1/8"]'),
    quarterFinal: document.querySelector('.playoff button[value="1/4"]'),
    semiFinal: document.querySelector('.playoff button[value="1/2"]'),
    final: document.querySelector('.playoff button[value="final"]'),
};
let playOfftable = document.querySelector('.playoff-section .playoff__flex');
playoffBtns.final.addEventListener('click', function () {
    playOfftable.style.transform = 'translateX(calc(-100% + 410px))'
})

playoffBtns.semiFinal.addEventListener('click', function () {
    playOfftable.style.transform = 'translateX(calc(-100% + 860px))'
})

playoffBtns.quarterFinal.addEventListener('click', function () {
    playOfftable.style.transform = 'translateX(calc(-100% + 1305px))'
})

playoffBtns.eighthFinal.addEventListener('click', function () {
    playOfftable.style.transform = 'translateX(calc(-100% + 1752px))'
})

playoffBtns.sixteenthFinal.addEventListener('click', function () {
    playOfftable.style.transform = 'translateX(0)'
})

const tournamentBtns = {
    sixteenthFinal: document.querySelector('.tournament button[value="1/16"]'),
    eighthFinal: document.querySelector('.tournament button[value="1/8"]'),
    quarterFinal: document.querySelector('.tournament button[value="1/4"]'),
    semiFinal: document.querySelector('.tournament button[value="1/2"]'),
    final: document.querySelector('.tournament button[value="final"]'),
};
let tournamentTable = document.querySelector('.tournament .playoff__flex');
tournamentBtns.final.addEventListener('click', function () {
    tournamentTable.style.transform = 'translateX(calc(-100% + 410px))'
})

tournamentBtns.semiFinal.addEventListener('click', function () {
    tournamentTable.style.transform = 'translateX(calc(-100% + 860px))'
})

tournamentBtns.quarterFinal.addEventListener('click', function () {
    tournamentTable.style.transform = 'translateX(calc(-100% + 1305px))'
})

tournamentBtns.eighthFinal.addEventListener('click', function () {
    tournamentTable.style.transform = 'translateX(calc(-100% + 1752px))'
})

tournamentBtns.sixteenthFinal.addEventListener('click', function () {
    tournamentTable.style.transform = 'translateX(0)'
})
let resultMatchModal = document.getElementById('resultMatchModal');


function openResultMatchModal() {
        modals.forEach(function (modal) {
        modal.style.display = 'none';
    })
    resultMatchModal.style.display = 'block';
    document.querySelector('body').style.overflow = 'hidden'
    changeBurger();
      scrollToTop();
      footerMenu.classList.remove('active');
...
const dateSwiper3 = document.querySelector('.date-swiper-3');
    ...
    if (dateSwiper3) {
        const wrapper = dateSwiper3.querySelector('.swiper-wrapper');
        wrapper.innerHTML = '';
      ...
      `;
        ...
        var Swipes = new Swiper('.date-swiper-3', {
            loop: false,
            spaceBetween: 9,
            slidesPerView: 'auto',
            navigation: {
                nextEl: '.date-next-btn-3',
                prevEl: '.date-prev-btn-3',
                clickable: true,
            },
        });
    }
    ...
    if (reviewsSwiper) {
        var Swipes = new Swiper('.reviews__swiper', {
            loop: false,
            spaceBetween: 20,
            slidesPerView: 'auto',
        });
...
const dateSwiper2 = document.querySelector('.date-swiper-2');
    ...
    if (dateSwiper2) {
        const wrapper = dateSwiper2.querySelector('.swiper-wrapper');
        wrapper.innerHTML = '';
      ...
      `;
        ...
        var Swipes = new Swiper('.date-swiper-2', {
            loop: false,
            spaceBetween: 9,
            slidesPerView: 'auto',
            navigation: {
                nextEl: '.date-next-btn-2',
                prevEl: '.date-prev-btn-2',
                clickable: true,
            },
        });
    }
...
let videoSwiper = document.querySelector('.video-modal__swiper');

if (videoSwiper) {
    var Swipes = new Swiper('.video-modal__swiper', {
        loop: false,
        spaceBetween: 8,
        slidesPerView: 'auto',
    });
...
let monthSwiper = document.querySelector('.month-swiper');

if (monthSwiper) {
    var Swipes = new Swiper('.month-swiper', {
        loop: true,
        spaceBetween: 18,
        slidesPerView: 3,
        navigation: {
            nextEl: '.month-swiper__next',
            prevEl: '.month-swiper__prev',
            clickable: true,
        },
    });
}